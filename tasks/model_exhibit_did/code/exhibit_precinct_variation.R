...
plot <- ggplot(agg_results_df, aes(x = precinct, y = att, ymin = lower_ci, ymax = upper_ci, color = Group, shape = Group)) +
    geom_point(size = 3) +
    geom_errorbar() +
    ...
    scale_color_manual(values = c("blue","red"), labels = c("Least supporting precincts", "Most supporting precincts")) +
