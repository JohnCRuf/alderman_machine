...
# ward list
treatment_list <- list(c(11, 2022), c(22, 2019), c(25, 2019), c(20, 2019), c(23, 2018))
...
treatment_ward_list <- c(11, 22, 25, 20, 23)
...
supporting_precinct_list <- support_df %>%
  ...
  ward %in% control_ward_list | ward %in% treatment_list)
...
treatment_df <- tibble(ward = c(treatment_ward_list, control_ward_list), 
                       year_treat = c(2022, rep(2019, 3), 2018, rep(2019, length(control_ward_list))),
                       treatment = c(rep(1, 5), rep(0, 13)))
...
save(menu_df, top_precincts, bottom_precincts, treatment_df, file = paste0("../output/corruption_", args[1] ,"precincts.rda"))