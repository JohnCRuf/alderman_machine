...
library(units)
library(assertthat)
source("../input/compute_border_distances.R")

compute_needs <- function(model,ward_map) {
    #add areas to ward map
    ward_map$area <- st_area(ward_map$geometry) %>% set_units(mi^2)
    #get model predictions for ward map areas
    ward_map$pct_of_needs <- predict(model, newdata = ward_map)
    #add year range that is just 2012-2022
    ward_map$year_range <- "2012-2022"
}

prepare_border_data <- function(ward_map, block_map, spending_map) {
    ward_map_2012 <- ward_map
    block_map_2010 <- block_map
    ...
    needs_df <- compute_needs(ward_map_2012)
    needs_df <- needs_df %>% 
    ...
    spending_2012_2022 <- spending_map
