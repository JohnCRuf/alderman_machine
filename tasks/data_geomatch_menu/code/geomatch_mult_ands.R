...
df <- read_csv("../input/geocoded_mult_ands_df.csv")
...
df_points <- df_points %>%
  rowwise() %>%
  mutate(lat = sum(c_across(starts_with("lat_")), na.rm = TRUE),
         long = sum(c_across(starts_with("lon_")), na.rm = TRUE)) %>%
  ungroup() 


#feed single addresses into geomatch_single_coordinate if there are any
...
#if there are no lines, skip this step
if (nrow(df_line) > 0) {
  df<- df %>%
    anti_join(df_line)
...
#if there are no shapes, skip this step
if (nrow(df_shape) >0) {
df_shape_sf <- create_sf_geometry(df_shape, "lat", "lon", 4326)

df_shape_geomatched <- geomatch_shapes(df_shape_sf, map, 10)
