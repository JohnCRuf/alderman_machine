...
source("filter_to_cat.R")
ARGS<- commandArgs(trailingOnly = TRUE)
map_filename <- paste0("../temp/ward_precincts_", ARGS[2], "/ward_precincts_", ARGS[2], ".shp")
output_filename_rds <- paste0("../output/ward_precinct_menu_panel_", ARGS[2],"_", ARGS[1], ".rds")
output_filename_csv <- paste0("../output/ward_precinct_menu_panel_", ARGS[2],"_", ARGS[1], ".csv")
#load the map
...
for (i in 3:length(ARGS)) {
  ...
  if ("total_length" %in% colnames(df) & "intersect_length" %in% colnames(df)) {
    df <- df %>%
      select(location,type, ward, year, ward_locate, precinct_locate, id, est_cost, total_length, intersect_length) %>%
    ...
    df <- df %>%
      select(location,type, ward, year, ward_locate, precinct_locate, id, est_cost, total_area, intersect_area) %>%
      mutate(weight = intersect_area/total_area) %>%
      mutate(shape_marker = "area") %>%
      select(-total_area, -intersect_area)
  } else {
    df <- df %>%
      select(location, type,ward, year, ward_locate, precinct_locate, id, est_cost) %>%
      mutate(weight = 1,
             shape_marker = "point")
  }
...
#use generate regex command to generate the relevant category regex
regex <- generate_regex(ARGS[1])
#print regex to text file
writeLines(regex, "../output/regex.txt")
#use filter_type to filter df_append to only rows where type contains the regex
#save number of rows before filtering
nrow_before <- nrow(df_append)
df_append <- filter_type(df_append, regex)
nrwo_after <- nrow(df_append)
#assert that the number of rows after filtering is less than the number of rows before filtering
assert_that(nrwo_after < nrow_before)
