...
def contracts_to_parquet():
    pd.read_csv('../../data/raw/data.cityofchicago.org/Contracts.csv').to_parquet('../../data/temp/contracts.parquet', index=False)

def geocode_contract_addresses():
    contracts = pd.read_parquet('../../data/temp/contracts.parquet')

    contracts = fixcols(contracts)
    ...
    with open('/Users/thomas/.keys/mapbox_be.key', 'r') as infile:
